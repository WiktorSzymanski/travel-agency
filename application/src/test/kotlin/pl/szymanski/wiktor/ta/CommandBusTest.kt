...
class CommandBusTest {
    ...
    @Test
    fun `dispatch should throw if no handler registered for command superclass`() = runTest {
        // Given
    ...
    @Test
    fun `registerHandler should enable dispatch for subclass using superclass key`() = runTest {
        // Given
        CommandBus.registerHandler(ADummyCommand::class.java) { command: ADummyCommand ->
            assertIs<OperationOfADummyCommand>(command)
            "OK" to listOf(DummyEvent(correlationId = command.correlationId))
    ...
    @Test
    fun `setup should wire handlers for all command groups and dispatch correctly`() = runTest {
        // Given
        ...
        // When
        val (_, toEvents) = CommandBus.dispatch<TravelOfferCommand, TravelOffer>(
        ...
        val (_, bEvents) = CommandBus.dispatch<BookingCommand, Booking>(
        ...
        val (_, cEvents) = CommandBus.dispatch<CommuteCommand, Commute>(
        ...
        val (_, aEvents) = CommandBus.dispatch<AttractionCommand, Attraction>(
        ...
        val (_, accEvents) = CommandBus.dispatch<AccommodationCommand, Accommodation>(
