...
class BookingSagaNewTest {
    ...
    private fun registerCommuteHandler(
        onCall: (CommuteCommand) -> Pair<Commute, List<Event>>
    ) {
    ...
    private fun registerAccommodationHandler(
        onCall: (AccommodationCommand) -> Pair<Accommodation, List<Event>>
    ) {
    ...
    private fun registerAttractionHandler(
        onCall: (AttractionCommand) -> Pair<Attraction, List<Event>>
    ) {
        CommandBus.registerHandler(AttractionCommand::class.java) { c -> onCall(c) }
    }

    @Test
    fun `saga success with attraction should complete and publish events`() = runTest {
        // Given
    ...
    @Test
    fun `saga success without attraction should complete and not call attraction handler`() = runTest {
        // Given
        val commuteAgg = mockk<Commute>(relaxed = true)
        val accommodationAgg = mockk<Accommodation>(relaxed = true)
    ...
    @Test
    fun `saga commute failure should publish failed and stop`() = runTest {
        // Given
    ...
    @Test
    fun `saga commute failure without attraction should publish failed and stop`() = runTest {
        // Given
        val triggering = reservedEvent(attractionId = null)
        val travelOfferService = mockk<TravelOfferService>(relaxed = true)
    ...
    @Test
    fun `saga accommodation failure should compensate commute and publish failed`() = runTest {
        // Given
        val commuteAgg = mockk<Commute>(relaxed = true)
        val accommodationAgg = mockk<Accommodation>(relaxed = true)
        ...
        registerCommuteHandler { command ->
            when (command) {
                is BookCommuteCommand -> {
                    commuteBookedEvent = CommuteBookedEvent(
                        commuteId = command.commuteId,
                        bookingId = command.bookingId,
                        seat = command.seat!!,
                        correlationId = command.correlationId,
    ...
    @Test
    fun `saga attraction failure should compensate accommodation and commute and publish failed`() = runTest {
        // Given
        val commuteAgg = mockk<Commute>(relaxed = true)
        val accommodationAgg = mockk<Accommodation>(relaxed = true)
        val attractionAgg = mockk<Attraction>(relaxed = true)
        ...
        registerAccommodationHandler { command ->
            when (command) {
                is BookAccommodationCommand -> {
                    accommodationBookedEvent = AccommodationBookedEvent(
                        accommodationId = command.accommodationId,
                        bookingId = command.bookingId,
                        correlationId = command.correlationId,
                    )
