...
class CommuteCommandHandler(
    ...
    suspend fun compensate(event: CommuteEvent) {
        EventBus.publish(when (event) {
            is CommuteBookedEvent -> handle(CancelCommuteBookingCommand(event.commuteId, event.correlationId!!, event.userId, event.seat))
            is CommuteBookingCanceledEvent -> handle(BookCommuteCommand(event.commuteId, event.correlationId!!, event.userId, event.seat))
