...
class AccommodationCommandHandler(
    ...
    suspend fun compensate(event: AccommodationEvent) {
        EventBus.publish(when (event) {
            is AccommodationBookedEvent -> handle(CancelAccommodationBookingCommand(event.accommodationId, event.correlationId!!, event.userId))
            is AccommodationBookingCanceledEvent -> handle(BookAccommodationCommand(event.accommodationId, event.correlationId!!, event.userId))
