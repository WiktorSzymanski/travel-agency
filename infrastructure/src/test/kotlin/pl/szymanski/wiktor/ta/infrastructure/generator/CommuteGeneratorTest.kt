...
class CommuteGeneratorTest {
    ...
    fun `generate commute from yaml`() {
        ...
        assert(
            actual[0].departure.time.isBetween(
        ...
        assert(
            actual[0].arrival.time.isBetween(
                actual[0].departure.time,
        ...
        assert(
            actual[1].departure.time.isBetween(
                fixedTime.plusSeconds(inAdvanceSeconds),
                fixedTime.plusSeconds(
                    inAdvanceSeconds + creationWindowSeconds / 2,
                ),
        ...
        assert(
            actual[1].arrival.time.isBetween(
                actual[1].departure.time,
                fixedTime.plusSeconds(inAdvanceSeconds + creationWindowSeconds),
            ),
        )
    }
