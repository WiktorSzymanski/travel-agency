...
class AccommodationGeneratorTest {
    ...
    fun `generate accommodation from yaml`() {
        ...
        assert(actual[0].rent.from.isBetween(fixedTime, fixedTime.plusSeconds(plusSeconds / 2)))
        assert(actual[0].rent.till.isBetween(actual[0].rent.from, fixedTime.plusSeconds(plusSeconds)))

        assertEquals("Hostel", actual[1].name)
        assertEquals(LocationEnum.BERLIN, actual[1].location)
        assert(actual[1].rent.from.isBetween(fixedTime, fixedTime.plusSeconds(plusSeconds / 2)))
        assert(actual[1].rent.till.isBetween(actual[1].rent.from, fixedTime.plusSeconds(plusSeconds)))
    }
}
